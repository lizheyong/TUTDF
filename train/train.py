...
def train_net(net, train_dataset, device, batch_size, lr, epochs):
    ...
    for epoch in range(epochs):
        ...
        for curve, label in train_loader:
            ...
            if loss < best_loss:
                best_loss = loss
                torch.save(net.state_dict(), fr"C:\Users\zheyong\Desktop\高光谱目标检测报告\铁测试\{this}\训练\resnet_{this}.pth")
            # 更新参数
            loss.backward()
            optimizer.step()
            # i += 1
...
if __name__ == "__main__":
    ...
    dataset = train_Loader(fr"C:\Users\zheyong\Desktop\高光谱目标检测报告\铁测试\{this}\训练\{this}_train.npy",
                         fr"C:\Users\zheyong\Desktop\高光谱目标检测报告\铁测试\{this}\训练\{this}_train_label.npy")

    train_net(net=ResNet, train_dataset=dataset, device=device,
              batch_size=1024, lr=0.0001, epochs=500)
